...
def _ignore_error(exception):
    return getattr(exception, "errno", None) in _IGNORED_ERRNOS or getattr(exception, "winerror", None) in _IGNORED_WINERRORS
...
class _WindowsFlavour(_Flavour):
    ...
    reserved_names = (
        {"CON", "PRN", "AUX", "NUL", "CONIN$", "CONOUT$"} | {"COM%s" % c for c in "123456789\xb9\xb2\xb3"} | {"LPT%s" % c for c in "123456789\xb9\xb2\xb3"}
    )
...
class Path(PurePath):
    ...
    def copytree(self, target, symlinks=False, ignore=None, copy_function=shutil.copy2, ignore_dangling_symlinks=True, dirs_exist_ok=False):
        ...
        if not dirs_exist_ok:
            shutil.copytree(
                src=self, dst=target, symlinks=symlinks, ignore=ignore, copy_function=copy_function, ignore_dangling_symlinks=ignore_dangling_symlinks
            )
        else:
